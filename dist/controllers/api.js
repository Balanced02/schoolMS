...
var _PayRoll = require('../models/PayRoll');
...
var AddPayHead = exports.AddPayHead = function AddPayHead(req, res) {
  ...
  if (_id) {
    _PayRoll2.default.findOneAndUpdate({ _id: _id }, {
      $set: _extends({}, req.body)
    }, {
      new: true
    }).then(function (data) {
    ...
    _PayRoll2.default.create(_extends({}, req.body)).then(function (data) {
      res.json(data);
    }).catch(function (err) {
      console.log(err);
      res.status(500).json({
...
var GetPayHead = exports.GetPayHead = function GetPayHead(req, res) {
  _PayRoll2.default.find().then(function (data) {
    return res.json(data);
  }).catch(function (err) {
    console.log(err);
    res.status(500).json({
